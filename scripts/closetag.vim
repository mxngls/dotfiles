...
function! GetLastOpenTag(unaryTagsStack)
	...
	while (!s:EmptystackP("b:lineTagStack"))
	    ...
	    elseif s:EmptystackP("b:TagStack") && !s:InstackOA(tag, a:unaryTagsStack)	"found unclosed tag
		return tag
	    else
...
" Returns closing tag for most recent unclosed tag, respectOAing the
...
" Remove and return the first element in OAthe stack
